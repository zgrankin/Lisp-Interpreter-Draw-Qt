...
void Expression::defineMethod()
{
	...
	arithmeticMethods(skipLast);
	booleanLogicMethods(skipLast);
	comparisonOperatorMethods(skipLast);
	trigMethods(skipLast);
	drawMethods(skipLast);
	
	if (atom.atomType == SymbolType && skipLast == false)
	{
...
Expression Expression::evaluateTree() {
	if (atom.var == "begin"){
		...
		else throw InterpreterSemanticError("Error: Cannot evaluate."); }
	else if (atom.var == "if"){
		if (children.size() != 3)
		...
		else throw InterpreterSemanticError("Error: Cannot evaluate. First expression of \"if\" did not return a bool type.");
	}
		...
		for (unsigned int i = 0; i < children.size(); i++) {
			if (children[i]->atom.atomType == SymbolType) {
				...
				if (children[i]->atom.atomType == NoneType) throw InterpreterSemanticError("Error: Unknown operation.");
			}
		...
		for (unsigned int i = 0; i < children.size(); i++) {
			...
			else { euthanizeChildren(); throw InterpreterSemanticError("Error: Cannot evaluate."); }
		}
		setDrawAtom();
	}
...
void Expression::arithmeticMethods(bool &skipLast)
{
...
void Expression::booleanLogicMethods(bool &skipLast)
{
	if (atom.var == "and" && children.size() != 0) {
...
void Expression::comparisonOperatorMethods(bool &skipLast)
{
	if (atom.var == "<") {
		if (children.size() == 2 && children[0]->atom.atomType == NumberType && children[1]->atom.atomType == NumberType) {
...
void Expression::trigMethods(bool &skipLast)
{
	if (atom.var == "sin") {
...
void Expression::drawMethods(bool &skipLast)
{
	if (atom.var == "point") {
		skipLast = true;
